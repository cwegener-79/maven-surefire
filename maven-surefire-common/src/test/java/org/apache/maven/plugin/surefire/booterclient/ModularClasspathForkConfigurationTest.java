...
public class ModularClasspathForkConfigurationTest {
    ...
    public void shouldCreateModularArgsFile() throws Exception {
        ...
        List<String> modulePath = asList("modular.jar", "target" + separatorChar + "classes");
        ...
        File jigsawArgsFile = config.createArgsFile(
                "abc",
                modulePath,
                classPath,
                packages,
        ...
        assertThat(argsFileLines.get(5)).isEqualTo("abc=\"" + replace(patchFile.getPath(), "\\", "\\\\") + "\"");
        ...
        ModularClasspathConfiguration modularClasspathConfiguration = new ModularClasspathConfiguration(
                modularClasspath, testClasspathUrls, surefireClasspathUrls, emptyClasspath(), true, true);
        ClassLoaderConfiguration clc = new ClassLoaderConfiguration(true, true);
        StartupConfiguration startupConfiguration = new StartupConfiguration(
                "JUnitCoreProvider", modularClasspathConfiguration, clc, null, Collections.<String[]>emptyList());
        Commandline cli = new Commandline();
        config.resolveClasspath(
                cli,
